...
# Parameters
###############################
layout = BIDSLayout('/Users/matteo/Downloads/2020_embcp')
...
for p in part:
    ...
    for epo_idx in range(len(epochs_clean)):
        ...
        epo_type = list(events_dict.keys())[list(
            events_dict.values()).index(epo.events[0][2])]
        ...
        if epo_type != 'rest_start':
            ...
            behav_info = mne.create_info(
                ['pain_rate'], raw.info['sfreq'], ['misc'])
            prate_chan = mne.io.RawArray(np.expand_dims(
                np.asarray(behav['pain_rating']), 0), info=behav_info)
            prate_chan.info['highpass'] = raw.info['highpass']
            prate_chan.info['lowpass'] = raw.info['lowpass']

            behav_info = mne.create_info(
                ['audio_rate'], raw.info['sfreq'], ['misc'])
            audio_chan = mne.io.RawArray(np.expand_dims(
                np.asarray(behav['audio_rating']), 0), info=behav_info)
            audio_chan.info['highpass'] = raw.info['highpass']
            audio_chan.info['lowpass'] = raw.info['lowpass']

            behav_info = mne.create_info(
                ['stim_int'], raw.info['sfreq'], ['misc'])
            stim_chan = mne.io.RawArray(np.expand_dims(
                np.asarray(behav['stim_intensity']), 0), info=behav_info)
        ...
        for e in epo.drop_log:
            if e:
                for c in e:
                    ...
                    rej_stats.loc[c, 'total_chan'] = rej_stats.loc[c,
                                                                   'total_chan'] + 1
    ...
    # Save report
    rej_stats.sort_values(['total_chan']).to_csv(
        opj(derivpath, p + '_drop100.csv'))
