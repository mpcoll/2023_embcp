...
def read_tf(df, idx, scorename, regression, transform, scale_param):
    ...
    eeg = np.reshape(eeg, (1, eeg.shape[0], eeg.shape[1], eeg.shape[2]))
    X = np.float32(eeg)
    if regression:
...
class EEGdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X, y = read_tf(self.df, idx, self.scorename, self.regression,
...
def prefetch_map(func, input_iter, prefetch=1, check_interval=5):
    ...
    mapper_thread = threading.Thread(target=_mapper_loop, args=(
...
def generate_validation_model(df_tr, df_val, scorename, model,
    ...
    if x_transform == 'standardscaler':
        ...
        for batch in tqdm(trainloader, total=len(trainset)//batch_size):
            batch_flat = batch[0].squeeze().reshape((batch[0].shape[0],
                                                     batch[0].squeeze().shape[1]*batch[0].squeeze().shape[2]*batch[0].squeeze().shape[3]))

            scaler.partial_fit(batch_flat)

        scale_param = [scaler.mean_.reshape((batch[0].shape[2:])), np.sqrt(
            scaler.var_).reshape((batch[0].shape[2:]))]
        # Load raw data
        trainset = EEGdataset(df_tr, scorename, regression=regression,
    ...
    for epoch in range(n_epochs):
        ...
        print('scorename: ' + scorename + '_Iter '+str(0)+' Epoch '+str(epoch)
              + ' Val. Acc.: '+str(np.round(valid_acc, 2))
                + ' Bal. Val. Acc.: '
                + str(np.round(bal_valid_acc, 2)) +
              ' Tr. Loss ' + str(np.round(loss, 2))
...
def generate_validation_model_regression(df_tr, df_val, scorename, model,
    ...
    # Get the training and validation data
    trainset = EEGdataset(
        df_tr, scorename, regression=True, transform=x_transform)
    ...
    validset = EEGdataset(
        df_val, scorename, regression=True, transform=x_transform)
    validloader = DataLoader(validset, batch_size=batch_size, shuffle=True,
                             num_workers=num_workers, drop_last=True)
    ...
    for epoch in range(n_epochs):
        ...
        print('Training epoch ' + str(epoch))
        t0 = time.time()
        loss = train(trainloader, net, optimizer,
                     criterion, cuda_avl, len(trainset))
        loss = loss.data.cpu().numpy()
        # Do not get training acc (not useful?)
        # y_true, y_pred, _ = test(trainloader, net, cuda_avl, criterion)
        # train_acc = accuracy_score(y_true, y_pred)
...
def sensitivity_analysis(model, image_tensor, target_class=None,
    ...
    # Forward pass.
    # convert numpy or list to tensor        print(image_tensor.shape)
...
def area_occlusion(model, image_tensor, area_masks, target_class=None,
    ...
    for area_mask in area_masks:
        ...
        image_tensor_occluded = image_tensor * \
            (1 - area_mask).view(image_tensor.shape)

        output = model(
            Variable(image_tensor_occluded[None], requires_grad=False))[0]
        if apply_softmax:
            output = F.softmax(output)
...
def run_saliency(odir, itrpm, images, net, area_masks, iter_, scorename, taskM):
    for nSub in np.arange(images.shape[0]):
        print(nSub)
        fname = odir + itrpm + '_' + scorename + '_iter_' + \
            str(iter_) + '_nSub_' + str(nSub) + '.nii'
        if itrpm == 'AO':
            interpretation_method = area_occlusion
            sal_im = interpretation_method(
                net, images[nSub], area_masks, occlusion_value=0, apply_softmax=False, cuda=False, verbose=False, taskmode=taskM)
        elif itrpm == 'BP':
            interpretation_method = sensitivity_analysis
            sal_im = interpretation_method(
                net, images[nSub], apply_softmax=False, cuda=False, verbose=False, taskmode=taskM)
        else:
...
def evaluate_test_accuracy(model, outpath, df_te, num_classes, scorename,
    ...
    X_te, y_te = read_X_y_5D(df_te, scorename, regression=regression)
    ...
    if classify:
        ...
        outs.loc[0] = [mae_te, ev_te, mse_te,
                       r2_te, r_te,  rmse_te, bal_acc, acc_te]
    else:
        outs = pd.DataFrame(columns=['mae_te', 'expvar_te', 'mse_te', 'r2_te',
...
def run_SML_Classifiers(meth, x_tr, y_tr, x_va, y_va, x_te, y_te, trd, pp=0,
    ...
    param_grid_svmk = [
        {'C': C_range_ker, 'gamma': Y_range, 'coef0': coef0Vals}]

    if meth == 'LDA':
        clf = LinearDiscriminantAnalysis()
    elif meth == 'LR':
        gs = GridSearch(model=LogisticRegression(
        ), param_grid=param_grid_lr, parallelize=parallelization, num_threads=nt)
        ...
        clf = SVC(kernel="linear",
                  C=gs.best_params['C'], gamma=gs.best_params['gamma'], max_iter=mi)
    elif meth == 'SVMR':
        gs = GridSearch(model=SVC(kernel="rbf", max_iter=mi),
                        param_grid=param_grid_svmk, parallelize=parallelization, num_threads=nt)
        gs.fit(x_tr, y_tr, x_va, y_va)
        clf = SVC(
            kernel="rbf", C=gs.best_params['C'], gamma=gs.best_params['gamma'], max_iter=mi)
    elif meth == 'SVMP':
        gs = GridSearch(model=SVC(kernel="poly", degree=2, max_iter=mi),
                        param_grid=param_grid_svmk, parallelize=parallelization, num_threads=nt)
        gs.fit(x_tr, y_tr, x_va, y_va)
        clf = SVC(kernel="poly", C=gs.best_params['C'],
                  gamma=gs.best_params['gamma'], coef0=gs.best_params['coef0'], max_iter=mi)
    elif meth == 'SVMS':
        gs = GridSearch(model=SVC(kernel="sigmoid", max_iter=mi),
                        param_grid=param_grid_svmk, parallelize=parallelization, num_threads=nt)
        gs.fit(x_tr, y_tr, x_va, y_va)
        clf = SVC(kernel="sigmoid",
                  C=gs.best_params['C'], gamma=gs.best_params['gamma'], coef0=gs.best_params['coef0'], max_iter=mi)
    else:
...
def train_regression_tune(config):
    ...
    if config['model'] == 'AlexNet3D_Dropout_Regression':
        ...
        net = resnet152(
            num_classes=1, linear_size=config['constant']['linear_size'])
    ...
    # Get the training and validation data
    trainset = MRIDataset(config['constant']['df_tr'], config['constant']
                          ['scorename'], regression=True, transform=config['x_transform'])

    if config['constant']['imbalance_sampler']:
        sampler = WeightSampler(
            config['constant']['df_tr'][config['constant']['scorename']])
    ...
    validset = MRIDataset(config['constant']['df_val'], config['constant']
                          ['scorename'], regression=True, transform=config['x_transform'])
    ...
    for epoch in range(config['n_epochs']):
        ...
        y_true, y_pred = test_reg(
            validloader, net, config['constant']['cuda_avl'])
        valid_mae = mean_absolute_error(y_true, y_pred)
        valid_ev = explained_variance_score(y_true, y_pred)
        val_loss = mean_squared_error(y_true, y_pred)
        valid_r2 = r2_score(y_true, y_pred)
        ...
        history.loc[epoch] = [config['constant']['scorename'], epoch, valid_mae, valid_ev,
                              valid_r2, valid_r, loss, val_loss]
        history.to_csv(opj(config['constant']['outpath'],
                       'train_history.csv'), index=False)
            ...
            if valid_mae < min_val_mae:
                ...
                if epochs_no_improve == config['constant']['early_plateau']:
                    ...
                    plt.title('Running loss at early stop '
                              + str(epoch) + ' epochs')
                    fig.savefig(
                        opj(config['constant']['outpath'], 'train_history_early.png'))
...
def train_tune(config):
    ...
    if config['model'] == 'AlexNet3D_Dropout_Regression':
        ...
        net = AlexNet3D_Deeper_Dropout(
            num_classes=config['constant']['num_classes'])
    elif config['model'] == 'AlexNet3D_Dropout':
        net = AlexNet3D_Dropout(num_classes=config['constant']['num_classes'])
    elif config['model'] == 'ResNet_152':
        net = resnet152(num_classes=config['constant']['num_classes'],
                        linear_size=config['constant']['linear_size'])
    elif config['model'] == 'ResNet_50':
        net = resnet50(num_classes=config['constant']['num_classes'],
                       linear_size=config['constant']['linear_size'])
    elif config['model'] == 'ResNet_34':
        net = resnet34(num_classes=config['constant']['num_classes'],
                       linear_size=config['constant']['linear_size'])
    elif config['model'] == 'DenseNet_121_dropout':
        net = densenet121_3D_DropOut(
            num_classes=config['constant']['num_classes'])
    elif config['model'] == 'DenseNet_161_dropout':
        net = densenet161_3D_DropOut(
            num_classes=config['constant']['num_classes'])
    else:
        print('Check model')
    ...
    # Get the training and validation data
    trainset = MRIDataset(config['constant']['df_tr'], config['constant']
                          ['scorename'], regression=False, transform=config['x_transform'])
    ...
    for epoch in range(config['n_epochs']):
        ...
        y_true, y_pred, _ = test(
            validloader, net, config['constant']['cuda_avl'], criterion)
        valid_mae = mean_absolute_error(y_true, y_pred)
        valid_ev = explained_variance_score(y_true, y_pred)
        val_loss = mean_squared_error(y_true, y_pred)
        valid_r2 = r2_score(y_true, y_pred)
