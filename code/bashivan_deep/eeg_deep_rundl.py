...
datapath = opj(basepath, 'bashivan_deep', '3dssf_larger_cleaner')

df = pd.read_csv(opj(datapath, 'metadata.csv')).reset_index(drop=True)

df.to_csv('test.csv')
# Drop bad participants
df = df[~df.subject_id.isin(['sub-012', 'sub-015', 'sub-027'])]

# df = df[df.condition.isin(['thermalrate_start'])]
# df['pain_rating'][df['pain_rating'].isna()] = 0
# df['ispain'] = np.where((df['pain_rating'].astype(float) > 5)
#                         & (df['pain_rating'].astype(float) < 200), 1, 0)

df['ispain'] = np.where(df['condition'].isin(['thermal_start',
                                              'thermalrate_start']), 1, 0)
...
# Try individial participants
#TODO test the eeglearn models on the large fucking chunks
#TODO validate within participants
#TODO validate easy classificaitons, clearn more, try different preprocessing
#TODO check wich participants are not predicted good
from sklearn.model_selection import LeaveOneGroupOut, GroupShuffleSplit

cv = LeaveOneGroupOut().split(df, y=None, groups=df['subject_id'])


all_train_acc = []
all_test_acc = {}
for train, test in cv:
    df_trainval = df.iloc[train]
    df_test = df.iloc[test]

    # Randomly select 10% of participants in train as validation
    train_ids, val_ids = next(GroupShuffleSplit(n_splits=2,
                                                test_size=0.1).split(df_trainval,
                                                                     groups=df_trainval['subject_id']))

    df_train = df_trainval.iloc[train_ids]
    df_val = df_trainval.iloc[val_ids]
    ...
    history, max_val_acc = generate_validation_model(df_tr=df_train,
                                                                df_val=df_val,
                                                                scorename=scorename,
                                                                model=model,
                                                                ...
                                                                num_workers=0,
                                                                batch_size=32)
    all_train_acc.append(max_val_acc)

    outs, df_te = evaluate_test_accuracy(model,
                                        outpath,
                                        df_test,
                                        linear_size=1024,
                                        x_transform='zscore',
    ...
    all_test_acc[df_test.subject_id.iloc[0]] =  outs['bal_acc_all'].iloc[0]
