# Copied from https://github.com/VDelv/EEGLearn
...
class MaxCNN(nn.Module):
    ...
    return x: output of the last layers after the log softmax
    '''
    def __init__(self, input_image=torch.zeros(1, 7, 6, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=4):
...
class TempCNN(nn.Module):
    ...
    return x: output of the last layers after the log softmax
    '''
    def __init__(self, input_image=torch.zeros(1, 7, 6, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(3,3), n_classes=4):
...
class LSTM(nn.Module):
    ...
    return x: output of the last layers after the log softmax
    '''
    def __init__(self, input_image=torch.zeros(1, 7, 6, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(3,3), n_classes=4, n_units=128):
        super(LSTM, self).__init__()

        self.n_window = input_image.shape[1]
        self.n_channel = input_image.shape[2]

        self.conv1 = nn.Conv2d(self.n_channel,32,kernel,stride=stride, padding=padding)
        self.conv2 = nn.Conv2d(32,32,kernel,stride=stride, padding=padding)
        self.conv3 = nn.Conv2d(32,32,kernel,stride=stride, padding=padding)
        self.conv4 = nn.Conv2d(32,32,kernel,stride=stride, padding=padding)
        self.pool1 = nn.MaxPool2d(max_kernel)
        ...
        # LSTM Layer
        self.rnn = nn.RNN(4*4*128, n_units, self.n_window)
        self.rnn_out = torch.zeros(2, 7, 128)

        self.pool = nn.MaxPool2d((self.n_window,1))
        self.drop = nn.Dropout(p=0.5)
...
class Mix(nn.Module):
    '''
        ...
        return x: output of the last layers after the log softmax
        '''
    def __init__(self, input_image=torch.zeros(1, 7, 6, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=4, n_units=128):
        ...
        # Temporal CNN Layer
        self.conv8 = nn.Conv1d(self.n_window, 64, (4 * 4 * 128, 3), stride=stride, padding=padding)
